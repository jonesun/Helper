        ...
        try {
            ...
            new Thread() {
                public void run() {
                    ...
                    try {
                        if (isVideo) {
                            Video.Thumbnails.cancelThumbnailRequest(context.getContentResolver(), origId);
                        } else {
                            Images.Thumbnails.cancelThumbnailRequest(context.getContentResolver(), origId);
                        }
                    } catch (Exception e) {
                        ;
                    }
            ...
            if (isVideo) {
                bitmap = Video.Thumbnails.getThumbnail(context.getContentResolver(), origId,
                        Video.Thumbnails.MINI_KIND, null);
            } else {
                bitmap = Images.Thumbnails.getThumbnail(context.getContentResolver(), origId,
