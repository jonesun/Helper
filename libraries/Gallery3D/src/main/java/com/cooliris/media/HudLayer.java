...
public final class HudLayer extends Layer {
    ...
    private Menu[] mNormalBottomMenu = null;
    private Menu[] mSingleViewIntentBottomMenu = null;
    private Menu[] mNormalBottomMenuNoShare = null;
    private Menu[] mSingleViewIntentBottomMenuNoShare = null;
    ...
    HudLayer(Context context) {
        ...
        Menu shareMenu = new Menu.Builder(context.getResources().getString(Res.string.share)).icon(
        ...
        Menu deleteMenu = new Menu.Builder(context.getResources().getString(Res.string.delete)).icon(
                Res.drawable.icon_delete).options(deleteOptions).build();

        Menu moreMenu = new Menu.Builder(context.getResources().getString(Res.string.more)).icon(
        ...
        mNormalBottomMenu = new Menu[] { shareMenu, deleteMenu, moreMenu };
        mSingleViewIntentBottomMenu = new Menu[] { shareMenu, moreMenu };
        
        mNormalBottomMenuNoShare = new Menu[] { deleteMenu, moreMenu };
        mSingleViewIntentBottomMenuNoShare = new Menu[] { moreMenu };

        mSelectionMenuBottom.setMenus(mNormalBottomMenu);
        mSelectionMenuTop = new MenuBar(context);
        mSelectionMenuTop.setMenus(new Menu[] {
                new Menu.Builder(context.getResources().getString(Res.string.select_all)).onSelect(new Runnable() {
                    public void run() {
                        mGridLayer.selectAll();
                    }
                }).build(), new Menu.Builder("").build(),
                new Menu.Builder(context.getResources().getString(Res.string.deselect_all)).onSelect(new Runnable() {
                    public void run() {
                        mGridLayer.deselectOrCancelSelectMode();
                    }
                }).build() });
        mFullscreenMenu = new MenuBar(context);
        mFullscreenMenu.setMenus(new Menu[] {
                new Menu.Builder(context.getResources().getString(Res.string.slideshow)).icon(Res.drawable.icon_play)
                ...
                new Menu.Builder(context.getResources().getString(Res.string.menu)).icon(Res.drawable.icon_more)
                        .onSingleTapUp(new Runnable() {
                            public void run() {
                                if (getAlpha() == 1.0f)
    ...
    public void updateNumItemsSelected(int numItems) {
        ...
        Menu menu = new Menu.Builder(numItems + items).config(MenuBar.MENU_TITLE_STYLE_TEXT).build();
